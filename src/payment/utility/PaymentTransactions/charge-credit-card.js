...
function chargeCreditCard(customerObj,callback) {
...
//from here
var creditCard = new ApiContracts.CreditCardType();
	creditCard.setCardNumber(customerObj.cardNumber);  //'4242424242424242'
	creditCard.setExpirationDate(customerObj.expiryDate);   //'0835'
	creditCard.setCardCode(customerObj.cardCode);

	var paymentType = new ApiContracts.PaymentType();
	paymentType.setCreditCard(creditCard);

	var orderDetails = new ApiContracts.OrderType();
	orderDetails.setInvoiceNumber(customerObj.invoice);    //'INV-12345'
	orderDetails.setDescription(customerObj.description);

	var tax = new ApiContracts.ExtendedAmountType();
	tax.setAmount(customerObj.amount);  //double
	tax.setName(customerObj.name); //tax name
	tax.setDescription(customerObj.description);

	var duty = new ApiContracts.ExtendedAmountType();
	duty.setAmount(customerObj.amount);  //details
	duty.setName(customerObj.name);
	duty.setDescription(customerObj.description);

	var shipping = new ApiContracts.ExtendedAmountType();
	shipping.setAmount(customerObj.amount);
	shipping.setName(customerObj.name);
	shipping.setDescription(customerObj.description);

	var billTo = new ApiContracts.CustomerAddressType();
	billTo.setFirstNameA(customerObj.billTo.firstName);
...
for(let item of customerObj.items){
	...
	for(let item of customerObj.users){
	...
	for(let item of customerObj.settings){
